...
fit <- cforest(as.factor(Survived) ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked + Title + FamilySize + FamilyID,
               data = clean_train, controls=cforest_unbiased(ntree=2000, mtry=3))

Prediction <- predict(fit, clean_test, OOB=TRUE, type = "response")
submit <- data.frame(PassengerId = clean_test$PassengerId, Survived = Prediction)
write.csv(submit, file = "Output/conditional_inference_forest.csv", row.names = FALSE)
